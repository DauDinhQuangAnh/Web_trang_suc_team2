<?php
    ...
    while ($row = mysqli_fetch_array($query_lietke_dh)) {
        ...
        $fill = !$fill;
        $total_amount += $row['soluongmua'] * $row['giasp'];
    }

    $pdf->Cell($width_cell[0] + $width_cell[1] + $width_cell[2] + $width_cell[3], 10, 'Tổng số tiền:', 1, 0, 'R', true);
    $pdf->Cell($width_cell[4] + $width_cell[5], 10, number_format($total_amount), 1, 1, 'C', $fill);

    $pdf->Write(10, 'Cảm ơn bạn đã đặt hàng tại website của chúng tôi <3.');
    $pdf->Ln(10);
